...
class CustomDatasourceRelation(override val sqlContext : SQLContext, path : String, userSchema : StructType)
  extends BaseRelation with TableScan with PrunedScan with InsertableRelation with Serializable {
// with PrunedFilteredScan
  ...
  override def buildScan(): RDD[Row] = {
    ...
    println(sqlContext.sparkContext.wholeTextFiles(path).toString())
...
//
//    println("PrunedFilteredScan: buildScan called...")
//
//    val schemaFields = schema.fields
//    // Reading the file's content
//    val rdd = sqlContext.sparkContext.wholeTextFiles(path).map(f => f._2)
